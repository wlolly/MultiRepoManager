...
export default function OutboundOrderDetail() {
  const [, params] = useRoute("/outbound-order/:id");
  ...
  // 查询出库单详情
  const { data: order, isLoading: isLoadingOrder } = useQuery<OutboundOrder>({
    queryKey: [`/api/outbound-orders/${orderId}`],
    enabled: !!orderId,
    onSuccess: (data) => {
      // 设置订单更新表单的默认值
  ...
  const addItemMutation = useMutation({
    mutationFn: (data: ItemFormValues) => {
      ...
      if (product.stock < data.quantity) {
        throw new Error(`${t("insufficient_stock")} ${product.name}: ${product.stock} ${t("available")}`);
      }
      
      const weight = product.singleWeightKg * data.quantity;
      const volume = product.singleVolumeM3 * data.quantity;
      
      return apiRequest(`/api/outbound-orders/${orderId}/items`, {
        method: "POST",
        body: JSON.stringify({
    ...
    onSuccess: () => {
      ...
      itemForm.reset();
      queryClient.invalidateQueries({ queryKey: [`/api/outbound-orders/${orderId}`] });
      toast({
        title: t("item_added"),
  ...
  const deleteItemMutation = useMutation({
    mutationFn: (itemId: number) => {
      return apiRequest(`/api/outbound-order-items/${itemId}`, {
        method: "DELETE",
      });
    },
    onSuccess: () => {
  ...
  const updateOrderMutation = useMutation({
    mutationFn: (data: OrderUpdateFormValues) => {
      return apiRequest(`/api/outbound-orders/${orderId}`, {
        method: "PATCH",
        body: JSON.stringify({
          status: data.status,
          notes: data.notes,
    ...
    return (
      <div className="container mx-auto py-6 px-4 md:px-6 text-center">
        ...
        <Link href="/outbound-orders">
          <Button>{t("back_to_outbound_orders")}</Button>
        </Link>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <div className="flex items-center mb-2">
            ...
            <h1 className="text-3xl font-bold">{t("outbound_order")}: {order.orderNumber}</h1>
          </div>
        ...
        <div className="mt-4 md:mt-0 space-x-2 flex">
          <Dialog open={isUpdatingOrder} onOpenChange={setIsUpdatingOrder}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{t("update_outbound_order")}</DialogTitle>
                <DialogDescription>
                  {t("update_outbound_order_description")}
                </DialogDescription>
              </DialogHeader>
                              ...
                              <ScrollArea className="h-72">
                                {products.map((product) => (
                                  <SelectItem key={product.id} value={product.id.toString()}>
                                    {product.name} ({product.barcode}) - {t("stock")}: {product.stock}
                                  </SelectItem>
                                ))}
                              </ScrollArea>
                            </SelectContent>
