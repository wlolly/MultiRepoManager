...
export default function NewWarehouseTransfer() {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">新建调拨单</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-2xl">
        <div>
          <label>调拨单号</label>
          <Input {...register('referenceNumber')} required />
        </div>
        <div>
          <label>源仓库</label>
          <Select {...register('sourceWarehouseId')} required>
        ...
        <div>
          <label>目标仓库</label>
          <Select {...register('targetWarehouseId')} required>
            <option value="1">上海仓库</option>
            <option value="2">北京仓库</option>
          </Select>
        </div>
