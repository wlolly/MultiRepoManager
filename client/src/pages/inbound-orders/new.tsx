...
export default function NewInboundOrder() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: FormValues) => {
      return apiRequest(`/api/inbound-orders`, {
        method: "POST",
        body: JSON.stringify({
    ...
    onSuccess: (data) => {
      toast({
        ...
        description: t("inbound_order_created_successfully"),
      });
      // 导航到新创建的入库单详情页
      navigate(`/inbound-order/${data.id}`);
    },
    onError: (error) => {
      toast({
        title: t("error"),
        description: error.message || t("failed_to_create_inbound_order"),
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <div className="container mx-auto py-6 px-4 md:px-6">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-3xl font-bold">{t("create_inbound_order")}</h1>
      </div>
      ...
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            {t("new_inbound_order_description")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                    ...
                    <FormControl>
                      <div className="flex">
                        ...
                        <Button 
                          ...
                          onClick={() => form.setValue("orderNumber", `IN-${Date.now()}`)}
                        >
                          {t("generate_number")}
                        </Button>
                      </div>
