    ...
    try {
      ...
      for (const item of data.items) {
        ...
        if (product && product.stock < quantity) {
          stockErrors.push(`${product.name} 库存不足。当前库存: ${product.stock}, 需求: ${quantity}`);
        }
      }
      ...
      // 创建出库单
      const orderResponse = await apiRequest<any>("/api/outbound-orders", {
        method: "POST",
        body: JSON.stringify({
      ...
      // 如果创建成功，添加明细项
      if (orderResponse?.id) {
        const outboundOrderId = orderResponse.id;
        ...
        for (const item of data.items) {
          ...
          if (selectedProduct) {
            await apiRequest<any>(`/api/outbound-orders/${outboundOrderId}/items`, {
              method: "POST",
              body: JSON.stringify({
                outboundOrderId,
        ...
        // 创建成功后跳转到订单详情页
        navigate(`/outbound-order/${outboundOrderId}`);
      }
    } catch (error) {
  ...
  const handleProductChange = (value: string, index: number) => {
    ...
    if (selectedProduct) {
      ...
      if (quantity > selectedProduct.stock) {
        toast({
          title: "库存警告",
          description: `${selectedProduct.name} 当前库存为 ${selectedProduct.stock}，低于选择的数量 ${quantity}`,
          variant: "destructive",
        });
      }
    }
    ...
    <div className="container mx-auto py-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {t("new_outbound_order_with_items_description")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                      ...
                      {fields.map((field, index) => (
                        ...
                        >
                                    ...
                                    <SelectContent>
                                      {isLoadingProducts ? (
                                        ...
                                        products.map((product) => (
                                          ...
                                          >
                                            {product.name} - {product.barcode} (库存: {product.stock})
                                          </SelectItem>
                                        ))
                                      )}
                                    </SelectContent>
                          ...
                          {form.getValues(`items.${index}.productId`) && (
                            <div className="mt-2">
                              {(() => {
                                const productId = parseInt(form.getValues(`items.${index}.productId`));
                                const quantity = parseInt(form.getValues(`items.${index}.quantity`));
                                const product = products.find(p => p.id === productId);
                                
                                if (product) {
                                  const stockStatus = product.stock >= quantity
                                    ? { color: "green", message: "库存充足" }
                                    : { color: "red", message: "库存不足" };
                                  
                                  return (
                                    <Badge variant={stockStatus.color === "green" ? "default" : "destructive"}>
                                      {t("current_stock")}: {product.stock} ({stockStatus.message})
                                    </Badge>
                                  );
                                }
                      ...
                      <CardContent>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-lg">{t(`outbound_${form.getValues("orderType")}`)}</p>
                          </div>
                          <div>
                            <h4 className="font-medium text-gray-500">{t("destination_type")}</h4>
                            <p className="text-lg">{t(`destination_${form.getValues("destinationType")}`)}</p>
                          </div>
                          <div>
                            <h4 className="font-medium text-gray-500">{t("notes")}</h4>
                            <p className="text-lg">{form.getValues("notes") || "-"}</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="text-right">{t("stock_status")}</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {fields.map((field, index) => {
                              ...
                              return (
                                <TableRow key={field.id}>
                                  ...
                                  <TableCell className="text-right">
                                    {selectedProduct && (
                                      <Badge variant={selectedProduct.stock >= quantity ? "default" : "destructive"}>
