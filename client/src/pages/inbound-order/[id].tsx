...
export default function InboundOrderDetail() {
  ...
  const { 
    ...
    queryKey: ["/api/inbound-orders", id],
    enabled: !!id
  });
  ...
  const addItemMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/inbound-orders", id] });
      toast({
        title: t("item_added"),
  ...
  const deleteItemMutation = useMutation({
    mutationFn: (itemId: number) => {
      return apiRequest(`/api/inbound-orders/${id}/items/${itemId}`, {
        method: "DELETE",
      });
    },
    onSuccess: () => {
    ...
    return (
      <div className="container mx-auto py-6">
        <div className="flex flex-col items-center justify-center h-[50vh]">
          ...
          <Button onClick={() => navigate("/inbound-orders")}>{t("back_to_inbound_orders")}</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <Card>
          <CardHeader>
            <CardTitle>{t("inbound_order_details")}</CardTitle>
            <CardDescription>
              {t("inbound_order_details_description")}
            </CardDescription>
          </CardHeader>
          <CardContent>
