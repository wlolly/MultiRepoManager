...
export default function OutboundOrders() {
  ...
  // 获取出库单数据
  const { data: outboundOrders = [], isLoading: isLoadingOrders } = useQuery<OutboundOrder[]>({
  ...
  const downloadTemplate = () => {
    ...
    // 示例数据
    const exampleData = [
      ['OUB-20250314-0001', '1', 'pending', '示例数据，导入时请删除', '1', '高精度工业传感器', '10', '5.5', '0.03'],
      ['OUB-20250314-0002', '2', 'pending', '示例数据，导入时请删除', '2', '工业电机', '5', '15.0', '0.12'],
      ['', '', '', '请在此处填写您的数据...', '', '', '', '', '']
    ];
    ...
    // 添加工作表到工作簿
    XLSX.utils.book_append_sheet(wb, ws, t('outbound_orders_template'));
    
    // 添加说明工作表
    const instructionsData = [
      [t('outbound_orders_template_instructions')],
      [''],
    ...
    // 导出Excel文件
    XLSX.writeFile(wb, `${t('outbound_orders_template')}.xlsx`);
  ...
  const exportToExcel = () => {
    // 筛选数据
    const filteredData = outboundOrders
      .filter(order => 
    ...
    // 添加工作表到工作簿
    XLSX.utils.book_append_sheet(wb, ws, t('outbound_orders'));
  ...
  const processExcelImport = async () => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        // 上传数据到服务器
        const response = await fetch('/api/outbound-orders/import', {
          method: 'POST',
          headers: {
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t('outbound_orders')}</h1>
          <p className="text-muted-foreground">{t('manage_outbound_orders')}</p>
        </div>
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('import_outbound_orders')}</DialogTitle>
            <DialogDescription>
        ...
        <CardContent>
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex-[2] min-w-[300px]">
              ...
              <Input
                id="search"
                placeholder={t('search_outbound_orders')}
                value={searchQuery}
        ...
        <CardContent>
          {isLoadingOrders || isLoadingWarehouses ? (
            ...
            <div className="text-center py-10">
              <p className="text-muted-foreground">{t('no_outbound_orders')}</p>
            </div>
          ) : (
                  ...
                  {filteredOrders.map((order) => (
                    <TableRow key={order.id}>
                      <TableCell className="font-medium">
                        <Link href={`/outbound-order/${order.id}`} className="hover:underline text-blue-600">
                          {order.orderNumber}
                        </Link>
                      </TableCell>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem asChild>
                              <Link href={`/outbound-order/${order.id}`}>
                                {t('view_details')}
                              </Link>
                            </DropdownMenuItem>
