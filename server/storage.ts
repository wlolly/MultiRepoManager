...
export class MemStorage implements IStorage {
  ...
  async createWarehouseTransfer(insertTransfer: InsertWarehouseTransfer): Promise<WarehouseTransfer> {
    ...
    const createdAt = new Date();
    const transfer: WarehouseTransfer = { ...insertTransfer, id, createdAt };
  ...
  async getWarehouseTransfers(filter?: { sourceWarehouseId?: number, targetWarehouseId?: number, status?: string }): Promise<WarehouseTransfer[]> {
    ...
    // Sort by updatedAt (newest first)
    transfers.sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime());
  ...
  async getWarehouseTransferStats(): Promise<{
    ...
    const recentTransfers = transfers.filter(transfer => 
      transfer.createdAt >= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    ).length;
    ...
    return {
      ...
      totalWeight: transfers.reduce((sum, transfer) => sum + transfer.totalWeight, 0),
      totalVolume: transfers.reduce((sum, transfer) => sum + transfer.totalVolume, 0),
      recentTransfers
    };
  }

  async getWarehouseTransferItems(transferId: number): Promise<WarehouseTransferItem[]> {
    return Array.from(this.warehouseTransferItemsMap.values()).filter(
  ...
  async createWarehouseTransferItem(insertItem: InsertWarehouseTransferItem): Promise<WarehouseTransferItem> {
    ...
    const createdAt = new Date();
    const item: WarehouseTransferItem = { ...insertItem, id, createdAt };
  ...
  async trackUniqueCode(tracking: InsertUniqueCodeTracking): Promise<UniqueCodeTracking> {
    const id = this.uniqueCodeTrackingMap.size + 1;
    const createdAt = new Date();
